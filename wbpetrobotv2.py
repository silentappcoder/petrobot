...
if uploaded_file is not None:
    ...
    if uploaded_file.type == "application/pdf":
        try:
            ...
            st.success("Documento PDF processado com sucesso.")
        except Exception as e:
            st.error(f"Erro ao carregar o documento PDF: {e}")
            documentos = ""

    # Verifica se o arquivo é do tipo Excel
    elif uploaded_file.type in ["application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]:
        try:
            # Detecta o formato do Excel e utiliza o motor apropriado
            if uploaded_file.type == "application/vnd.ms-excel":  # .xls
            ...
            # Converte o conteúdo do Excel para texto
            documentos = df.to_csv(index=False)
            st.success("Documento Excel processado com sucesso.")
        except Exception as e:
            st.error(f"Erro ao carregar o documento Excel: {e}")
            documentos = ""
    else:
        st.error("Por favor, envie um arquivo PDF ou Excel válido.")
        documentos = ""
